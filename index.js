        ...
        upd(rowRef, d) {
            ...
            Object.entries(d).forEach(([k, v]) => {
                ...
                if (isOverwrite) {
                    ...
                    // ➕ APPEND MODE：追加新片段，但阻止模型重复回传整段旧内容造成指数式膨胀。
                    ...
                    if (!currentVal) {
                        ...
                        if (val === currentVal || currentVal.includes(val)) {
                            // 完全重复或已存在的片段：不写入。
                        } else if (val.includes(currentVal)) {
                            // 模型回传“旧全文 + 新内容”时直接采用较完整版本，避免旧全文再次追加。
                            row[k] = val;
                        } else {
                            row[k] += '；' + val;
                        }
    ...
    function gtb(s, ti) {
        ...
        if (!hasData) {
            ...
            const renderRow = (ri) => {
                ...
                // 1. 左侧行号列 (带行高拖拽)
                h += `<td class="g-col-num" style="${heightStyle}">
                <div class="g-n">
                ...
                h += `<td class="g-col-source" style="${heightStyle}" title="${esc(sourceRanges)}"><div class="lvm-row-source" title="${esc(sourceRanges)}">${esc(sourceRanges)}</div></td>`;
