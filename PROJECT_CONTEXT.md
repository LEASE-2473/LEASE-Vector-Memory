...
- `memory_lifecycle.js`：行级冷热、锁定、逐表 X、冷记忆书同步及“当前聊天记忆库”管理页。
- `backfill_manager.js`：自动批量填表、手动追溯和来源楼层范围。
- `vector_manager.js`：独立 API 设置页、统一知识书管理、Embedding、Rerank、检索及 `LEASE_Vector_Memory_Database` 持久化。
...
- 向量区固定为“当前聊天记忆库 / API 设置 / 知识书管理”三分区；主表逐行状态按钮始终可见，并支持工具栏批量显隐/锁定。关闭向量总开关会恢复所有冷行为白色，确保普通表格模式不丢注入。
...
- v4.1.1 已修复 GitHub 安装目录 `LEASE-Vector-Memory` 未被动态加载器识别、导致依赖链中断且顶部入口不创建的问题。顶部入口改用独立 `lvm-top-*` ID 和脑图标，初始化会等待 `#top-settings-holder` 出现；新版本已准备推送供 SillyTavern 更新。真实模型/API 与移动端交互仍待回归。
